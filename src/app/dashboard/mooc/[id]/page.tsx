                ...
                <div className="space-y-2">
                    {node.children?.map((child, childIndex) => (
                        <Card key={child.key} size="small" className="bg-white flex gap-2">
                            <span className={"mr-2"}>{child.key}</span>
...
export default function MoocCatalogue({params}: {
    ...
    const handleExpand = (keys: string | string[]) => {
        ...
        if (Array.isArray(keys)) {
            ...
            if (newKey) {
                // 检查是否已经有子节点数据
                const hasChildren = (nodes: TreeNode[]): boolean => {
                    for (const node of nodes) {
                        if (node.key === newKey) {
                            return Boolean(node.children && node.children.length > 0);
                        }
                        if (node.children) {
                            if (hasChildren(node.children)) return true;
                        }
                    }
                    return false;
                };
    ...
    return (
        <div className="flex flex-col h-full box-border overflow-hidden p-8">
            <div className="flex justify-between">
                ...
                <Button type={"primary"} onClick={()=> window.open(`/mooc/${params.id}/edit`)}>编辑目录</Button>
            </div>
            <Card title="目录">
                <Collapse
                    bordered={false}
